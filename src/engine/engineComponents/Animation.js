...
class Animation{
  ...
  constructor(aInfo = new Object()){
    this.#id = aInfo.id?aInfo.id:"undefined";
    this.#relatedTo = aInfo.relatedTo?aInfo.relatedTo:null; //the gO related to the animation
    this.#ease = aInfo.ease?aInfo.ease:"linear";
    this.#done = aInfo.done?aInfo.done:false;
    this.#loops = aInfo.loops?aInfo.loops*(aInfo.loopback?2:1):1;//by default 1 loop
    this.#reverse = aInfo.reverse?1:0;//if the animation are played in reverse
    this.#reversing = aInfo.reverse?1:0;//control Var
    this.#looped = 1;
    this.#infinite = aInfo.infinite?aInfo.infinite:false;//infinite looping
    this.#loopback = aInfo.loopback?1:0;//repeat the animation in reverse after end
    this.#duration = aInfo.duration?aInfo.duration:0;
    this.#elapsed = 0;
    this.#tElapsed = 0;//total time elapsed
    this.#enabled = aInfo.enabled?aInfo.enabled:false;
    this.#pendingTimersFix = false;
    this.#startedAt = NaN;
    this.#dStartedAt = NaN;
    this.#delay = aInfo.delay?aInfo.delay:NaN;
    this.#to = aInfo.to?aInfo.to:{};
    this.#onComplete = aInfo.onComplete?aInfo.onComplete:null;
    
    this.#keyframes = aInfo.keyframes?aInfo.keyframes:{};
